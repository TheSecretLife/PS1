  ...
  implements Listener
{
  ...
  @EventHandler
  public void onBlockPlace(BlockPlaceEvent e) { WorldGuardPlugin wg = (WorldGuardPlugin)Main.wgd;
    ...
    if (type > 0) {
      if (wg.canBuild(p, b.getLocation())) {
        if (p.hasPermission("protectionstones.create")) {
          ...
          Vector v1 = null;Vector v2 = null;
          blocktypedata = b.getType().toString() + "-" + b.getData();
          blocktype = b.getType().toString();
          if (type == 1) {
            if (StoneTypeData.RegionY(blocktypedata) == -1) {
              v1 = new Vector(bx - StoneTypeData.RegionX(blocktypedata), 0.0D, bz - StoneTypeData.RegionZ(blocktypedata));
          ...
          else if (StoneTypeData.RegionY(b.getType().toString()) == -1) {
            v1 = new Vector(bx - StoneTypeData.RegionX(blocktype), 0.0D, bz - StoneTypeData.RegionZ(blocktype));
          ...
          HashMap<Flag<?>, Object> newFlags = new HashMap();
          for (int i = 0; i < DefaultFlag.flagsList.length; i++) {
            for (int j = 0; j < Main.flags.size(); j++) {
              String[] rawflag = ((String)Main.flags.get(j)).split(" ");
              String flag = rawflag[0];
              String setting = ((String)Main.flags.get(j)).replace(flag + " ", "");
              if (DefaultFlag.flagsList[i].getName().equalsIgnoreCase(flag)) {
                if (setting != null) {
                  if ((DefaultFlag.flagsList[i].getName().equalsIgnoreCase("greeting")) || (DefaultFlag.flagsList[i].getName().equalsIgnoreCase("farewell"))) {
                    ...
                    newFlags.put(DefaultFlag.flagsList[i], Boolean.valueOf(true));
                  } else if (setting.equalsIgnoreCase("false")) {
                    newFlags.put(DefaultFlag.flagsList[i], Boolean.valueOf(false));
                  } else {
          ...
          if (StoneTypeData.AutoHide(blocktypedata).booleanValue()) {
  ...
  public void onPistonExtend(BlockPistonExtendEvent e) {
    ...
    if (pushedBlocks != null) {
      ...
      while (it.hasNext()) {
        ...
        if (type == 2) blocktypedata = b.getType().toString();
        if ((type > 0) && 
          (StoneTypeData.BlockPiston(blocktypedata))) {
          e.setCancelled(true);
        }
      }
    }
